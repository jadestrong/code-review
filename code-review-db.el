  ...
  (let ((code-version (oref-default 'code-review-db-database version))
    ...
    (closql-with-transaction db
      (when (= version 7)
        ...
        (closql db [:alter-table pullreq :add-column base-ref-name :default nil])
        (closql db [:alter-table pullreq :add-column head-ref-name :default nil])
...
(defun code-review-db-all-unfinished ()
  ...
  (when (and code-review-db-connection (closql-live-p code-review-db-connection))
    (closql-close code-review-db-connection)
